...
class System():
    ...
    def add_molecule(self, structure_file: str=None, keep_coordinates: bool=True, keep_box: bool=True):
        ...
        if keep_box:
            if extension=='pdb':
                ...
                print('WARNING: only PDB files are supported to include box data from structure files.' \
    ...
    def solvate(self, initial_conf: str, final_conf:str, 
        ...
        # check inserted solvent molecules looking for the line "Number of solvent molecules:" in gmx solvate output
        
        for iline,line in enumerate(result.split('\n')):
            if line.startswith('Number of solvent molecules'):
                n_added_molecules = int(line.strip().split()[-1])
        #print('got number of added water ({})'.format(n_added_molecules))
        
        self._last_saved_structure=final_conf

        print('Added {} solvent molecules'.format(n_added_molecules))
        print('Solvated structure saved conf in {} '.format(final_conf))


        # now add/create Molecule and Atom objects

        index_first_new_mol=len(self.molecules)
        index_last_new_mol=index_first_new_mol+n_added_molecules-1

        if initial_conf.split('.')[-1] != 'pdb':
            outname = initial_conf.split('/')[-1].split('.')[0]+'.pdb'
        else:
            outname = initial_conf.split('/')[-1]
        
        subprocess.run(['gmx', 'editconf', '-f', initial_conf, '-o',  outname], stdout=subprocess.PIPE,stderr=subprocess.PIPE).stderr.decode('utf-8')

        if solventbox=='spc216.gro':
            import inspect
        ...
        for imol in range(n_added_molecules-1):
            #print(imol+2)
            self.molecules.append(copy.deepcopy(self.molecules[-1]))
      
        index_first_new_mol=len(self.molecules)-1-n_added_molecules
        index_last_new_mol=len(self.molecules)-1
    ...
    def insert_molecules(self, molstruct: str, initial_conf: str, final_conf: str, 
        ...
        import subprocess        
        result=subprocess.run("{0} insert-molecules -f {1} -ci {2} -nmol {3} -o {4} -try 5  ".format("gmx",
                                                                                            initial_conf,
                                                                                            molstruct,
                                                                                            nmol,
                                                                                            final_conf),
